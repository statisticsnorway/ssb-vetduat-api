...
public class SsbVetduatServiceTest {
    ...
    void testSsbVetduatService_oneCode_notExists() throws Exception {
        ...
        when(accessTokenService.getAccessToken()).thenReturn("token123");
        when(vetduatRestRepository.callVetDuAt(anyString(), anyString(), anyString())).thenReturn(notExistsVetduatResult);
    ...
    void testSsbVetduatService_oneCode_exists() throws Exception {
        ...
        when(accessTokenService.getAccessToken()).thenReturn("token123");
        when(vetduatRestRepository.callVetDuAt(anyString(), anyString(), anyString())).thenReturn(TestResultStrings.singleExistsResult);
    ...
    @Test
    void testSsbVetduatService_accessToken_timeout() throws Exception {
        JsonNode expected = objectMapper.readTree(TestResultStrings.errorResultGtin23NoAccessToken);
        when(accessTokenService.getAccessToken()).thenReturn("");
        when(vetduatRestRepository.callVetDuAt(anyString(), anyString(), anyString())).thenReturn(TestResultStrings.singleExistsResult);
